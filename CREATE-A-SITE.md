...
- **The LLM key** — required. Its name must match `llm.apiKeyEnv` in
  `site.config.ts` — **`GROQ_API_KEY` by default** (`OPENROUTER_API_KEY` if
  you switch providers). Groq's free tier (https://console.groq.com)
  comfortably covers several sites on one key.
